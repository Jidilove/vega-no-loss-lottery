...
contract NoLossLotteryAdminTest is Test {
    ...
    function test_RevertWithdrawPrincipalTwice() public {
        vm.startPrank(alice);
        lottery.withdrawPrincipal(1);

        vm.expectRevert(NoLossLottery.PrincipalAlreadyWithdrawn.selector);
        lottery.withdrawPrincipal(1);
        vm.stopPrank();
    }

    function test_RevertClaimPrizeIfNotWinner() public {
        // random = 50e6 => winner is alice
        vm.prank(bob);
        vm.expectRevert(NoLossLottery.NotWinner.selector);
        lottery.claimPrize(1);
    }

    function test_RevertClaimPrizeTwice() public {
        vm.startPrank(alice);
        lottery.claimPrize(1);

        vm.expectRevert(NoLossLottery.PrizeAlreadyClaimed.selector);
        lottery.claimPrize(1);
        vm.stopPrank();
    }
    ...
    function test_SetEpochDurationBounds() public {
        lottery.setEpochDurationBounds(2 days, 20 days);

        assertEq(lottery.minEpochDuration(), 2 days);
        assertEq(lottery.maxEpochDuration(), 20 days);
    }
}
